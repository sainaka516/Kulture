      ...
      return initialTakes.map(newTake => {
        ...
        if (existingTake) {
          return {
            ...
            _count: {
              ...newTake._count,
              upvotes: existingTake.votes.filter(v => v.type === 'UP').length,
              downvotes: existingTake.votes.filter(v => v.type === 'DOWN').length,
            }
          }
        }
        return newTake
      ...
      const newTakes = prev.map(take =>
        take.id === updatedTake.id ? {
          ...
          _count: {
            ...take._count,
            ...updatedTake._count,
            upvotes: (updatedTake.votes || take.votes).filter(v => v.type === 'UP').length,
            downvotes: (updatedTake.votes || take.votes).filter(v => v.type === 'DOWN').length,
          }
        } : take
      )
